...
from dto.project import ProjectForm
...
class ProjectDao(Mapper):
    ...
    @classmethod
    async def add_project(cls, dto: ProjectForm, user_id: int):
        try:
            async with async_session() as session:
                async with session.begin():
                    data = await session.execute(
                        select(Project).where(Project.name == dto.name, Project.deleted_at == 0))
                    if data.scalars().first() is not None:
                        raise Exception("项目已存在")
                    pr = Project(**dto.dict(), create_user=user_id)
                    session.add(pr)
        except Exception as e:
            cls.__log__.error(f"新增项目: {dto.name}失败, {e}")
            raise Exception(f"新增项目: {dto.name}失败, {e}")
    ...
    @classmethod
    async def update_project(cls, dto: ProjectForm, role: int, user_id: int) -> None:
        """
        修改项目
        :return:
        """
        try:
            async with async_session() as session:
                async with session.begin():
                    ...
                    data.app = dto.app
                    # 如果修改人不是owner或者超管
                    if data.owner != dto.owner and role < Config.ADMIN and user_id != data.owner:
                    ...
                    data.updated_at = datetime.now()
                    data.update_user = user_id
                    data.dingtalk_url = dto.dingtalk_url
        except Exception as e:
            cls.__log__.error(f"编辑项目: {dto.name}失败, {e}")
            raise Exception(f"编辑项目: {dto.name}失败, {e}")
