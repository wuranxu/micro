...
from dto.testcase_directory import PityTestcaseDirectoryForm
from model.testcase_directory import PityTestcaseDirectory


@ModelWrapper(PityTestcaseDirectory)
class PityTestcaseDirectoryDao(Mapper):
            ...
            async with async_session() as session:
                async with session.begin():
                    sql = select(PityTestcaseDirectory).where(PityTestcaseDirectory.deleted_at == 0,
                                                              ...
                                                              PityTestcaseDirectory.parent == form.parent or None,
        ...
        for directory in res:
            if directory.parent is None:
                ...
                ans.append(dict(
                    ...
                    key=str(directory.id),
                    children=list(),
                ))
            else:
                parent_map[str(directory.parent)].append(directory.id)
    ...
    async def get_directory(ans_map: dict, parent_map, parent, children, case_map, case_node=None, move=False):
        ...
        for c in current:
            ...
            await PityTestcaseDirectoryDao.get_directory(ans_map, parent_map, str(temp.id), child, case_node, move=move)
