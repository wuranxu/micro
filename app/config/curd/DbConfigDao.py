...
from klose.model import async_session, db_helper
from klose.request.encoder import JsonEncoder
...
from dto.database import DatabaseDto
...
class DbConfigDao(Mapper):
    ...
    async def insert_database(data: DatabaseDto, user: str):
        try:
            async with async_session() as session:
    ...
    @staticmethod
    @RedisHelper.up_cache("database:cache")
    async def update_database(data: DatabaseDto, user: str):
        try:
            async with async_session() as session:
                async with session.begin():
    ...
    async def query_database_tree():
        ...
        try:
            ...
            result = []
            env_index = dict()
            env_data = await EnvironmentDao.list_env()
    ...
    async def get_tables(data: DatabaseDto):
